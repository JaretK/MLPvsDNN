...
drg_codes = map(str, [301, 302, 304, 310, 321])
...
def download(token, verbose=True):
    ...
    for id in dataset_ids:
        ...
        if year == 2011:
            filter_command ="""apr_drg_description_and_code='{}'"""
        # apr_drg_code are integers
        elif year == 2015 or year == 2016:
            filter_command ="""apr_drg_code={}"""
        else:
            filter_command ="""apr_drg_code='{}'"""
        # expand filter_command to include all drg_codes
        temp_command_list = []
        for x in drg_codes:
            temp_command_list.append(filter_command.format(x))
        filter_command = ' OR '.join(temp_command_list)
...
def main(argv):
    ...
    # print 'Loaded all patients: %s' % all_patients.shape[0]
